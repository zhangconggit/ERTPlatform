...
public class UIRoot :UIManager
{
    ...
    public Dictionary<string, Object> UIResources = new Dictionary<string, Object>();
    public Dictionary<string, Object> TxtResources = new Dictionary<string, Object>();
    public Dictionary<string, Object> PrefabResources = new Dictionary<string, Object>();
    ...
    public GameObject InstantiateCustom(string name, bool isOnline = true)
    {
        ...
        if (re != null && re.transform.parent == null)
        {
    ...
    public GameObject GetCustomObject(string _uName)
    {
        if (isOnline)
        {
            if (PrefabResources.ContainsKey(_uName))
            {
                return  PrefabResources[_uName] as GameObject;
            }
            return null;
        }
        else
    ...
    public Sprite GetCustomSprite(string _uName)
    {
        if (isOnline)
        {
            if (UIResources.ContainsKey(_uName))
            {
                return UIResources[_uName] as Sprite;
            }
            return null;
        }
        else
    ...
    public Texture2D GetCustomTexture(string _uName)
    {
        if (isOnline)
        {
            if (UIResources.ContainsKey(_uName))
            {
                return UIResources[_uName] as Texture2D;
            }
            return null;
        }
        else
