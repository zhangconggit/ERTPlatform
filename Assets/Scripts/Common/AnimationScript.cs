...
public class AnimationScript : AnimationManager
{
    ...
    public AnimationScript()
    {
        ...
        m_PlayBg = UIRoot.OnlineResources["zhuye-1" + "_T"] as Texture2D;
    }
    /// <summary>
    /// 接收处理消息
    /// </summary>
    ...
    void Start()
    {
        ...
        m_tPointImage = UIRoot.OnlineResources["qinang" + "_T"] as Texture2D; //缺数据
    ...
    IEnumerator Depack(string name, int k)
    {
        ...
        #region  手动控制帧动画
        {
            ...
            if (UIRoot.OnlineResources[name + "_T"] == null)
                yield break;
            Texture2D _texture = UIRoot.OnlineResources[name + "_T"] as Texture2D; //Resources.Load("IntubatingReverse/Intubating1") as Texture2D;
    ...
    /// </summary>
    /// <param name="modelName">模型名称</param>
    /// <param name="playOrStop">是否播放</param>
    /// <param name="speed">播放速度</param>
    /// <param name="startPos">初始位置</param>
    /// <param name="arrivePos">结束位置</param>
    /// <param name="index">动画索引</param>
    public void SetAnimaArriveState(GameObject modelName, bool playOrStop, float speed, float startPos = 0, float arrivePos = 100, int index = 0, VoidDelegate callBackEnd = null)
    {
        if (this.GetComponent<SetModelSkinValue>() == null)
        {
            this.gameObject.AddComponent<SetModelSkinValue>();
        }
        this.GetComponent<SetModelSkinValue>().PlayAnima(modelName, playOrStop, speed, startPos, arrivePos, index, callBackEnd);
    }
